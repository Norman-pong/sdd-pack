...
import type {
  ...
  ProposeOptions, ProposeResult,
  ArchiveOptions, ArchiveResult,
  MigrateOptions, MigrateResult,
  StatusResult, StatusItem,
  ListOptions, ListResult, ListItem,
  WhyResult,
  ApplyResult, ApplyChecklistItem,
} from "./lib/api-types";
import { validate, type ValidationConfig } from "./lib/validator";
import { parseDocument, parseStatusLine, parseStackedStatusLine, extractStatusLine, extractH1, isTemplateFile } from "./lib/doc-parser";
...
import {
  buildArchiveStatusLine, applyStatusLine, moveToArchive,
  appendReplacedByRef, appendSupersedesRef, syncIndex, mergeDelta,
...
function toValidationConfig(opts: ValidateOptions): ValidationConfig {
  ...
  if (opts.path) {
    ...
    else { docsDir = resolve("docs"); files = [p]; }
  }
  if (opts.staged) {
  ...
  return {
    docsDir, severity: opts.severity ?? "error",
...
export async function proposePrd(opts: ProposeOptions): Promise<ProposeResult> {
  ...
  try {
    ...
    if (opts.dryRun) {
      return { status: "pass", path: filePath, content, errors: [], warnings: [], next: "dry-run,未写入" };
    }
...
export async function archivePrd(opts: ArchiveOptions): Promise<ArchiveResult> {
  ...
  const r: ArchiveResult = {
    status: "pass", operations: [], statusLineUpdated: false,
    indexSynced: false, loreCommitted: false, errors: [], warnings: [],
  };
  try {
    const prdPath = requireFile(opts.prdPath);
    if (reason === "replaced" && !opts.newPrdPath) throw new Error("--reason replaced 需要 --new-prd");
    ...
    if (opts.dryRun) { r.operations.push(`(dry-run) 状态行 → ${reason}`, `(dry-run) index.md sync`); return r; }
    let content = applyStatusLine(readFileSync(prdPath, "utf-8"), buildArchiveStatusLine(reason, todayStr()));
    ...
    r.indexSynced = syncIndex(resolve("docs/index.md"), indexTarget, reason, prdPath.split("/").pop()?.replace(/\.md$/, "") ?? "");
    ...
    if (!opts.noCommit) {
      if (isLoreAvailable()) {
        const trailer = buildTrailer("archive", `归档 PRD: ${prdPath} (${reason})`, "docs", [prdPath]);
...
export async function migratePrd(opts: MigrateOptions): Promise<MigrateResult> {
  ...
  try {
    ...
    if (!statusLine) { r.warnings.push("未找到状态行"); return r; }
    const entries = parseStackedStatusLine(statusLine);
    if (!entries) { r.warnings.push("SKIP: 无法解析堆叠状态行"); return r; }
    ...
    let cl = `# ${name} 变更历史\n\n| 版本 | 日期 | 变更内容 |\n|------|------|----------|\n`;
    for (const e of entries) cl += `| ${e.version} | ${e.date ?? "日期待确认"} | ${e.description} |\n`;
...
export async function getStatus(): Promise<StatusResult> {
  ...
  for (const p of listMdFiles(resolve("docs/prd"))) {
    ...
    items.push({
      path: p, fileName: p.split("/").pop() ?? p, type: "prd",
      status: sl?.status ?? "(unknown)",
      version: sl?.version, publishDate: sl?.publishDate,
  ...
  for (const p of listMdFiles(resolve("docs/phase"))) {
    ...
    const sl = doc.statusLine ? parseStatusLine(doc.statusLine) : null;
    items.push({
      path: p, fileName: p.split("/").pop() ?? p, type: "phase",
      status: sl?.status ?? "(unknown)",
      references: doc.references.backRefs,
    });
  }
...
export async function listPrds(opts: ListOptions): Promise<ListResult> {
  ...
  const scan = (dir: string, type: string) => {
    for (const p of listMdFiles(dir)) {
      ...
      const sl = doc?.statusLine ? parseStatusLine(doc.statusLine) : null;
      items.push({
        date: dateFromPath(p), fileName: p.split("/").pop() ?? p,
        type, status: sl?.status ?? "(unknown)",
        title: extractH1(readFileSync(p, "utf-8")) ?? "(无标题)", path: p,
      });
    }
  };
  ...
  if (opts.keyword) {
    ...
    f = f.filter((i) => i.title.toLowerCase().includes(kw) || i.fileName.toLowerCase().includes(kw));
  }
...
export async function getApplyChecklist(prdPath: string): Promise<ApplyResult> {
  ...
  for (const line of content.split("\n")) {
    ...
    if (heading) { section = heading[2]; continue; }
