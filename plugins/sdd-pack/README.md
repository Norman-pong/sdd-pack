...
- **4 类 omp 资产齐备**: 1 skill + 5 rules + 3 agents + 1 extension(1 个 /sdd 主命令 + 18 子命令)
- **commit 门禁三段式**: TTSR 软门禁(rules) -> commit gate 硬门禁(`/sdd gate` 子命令) -> 三层守门 agent(reviewer / arch-reviewer / sdd-reviewer)
- **关键决策**: [ADR-009 sdd Extension 替代独立 CLI](../../docs/architecture/decisions.md) · [ADR-018 强状态流转 + meta.json 事实源](../../docs/architecture/decisions.md)

## 0.1 omp 组件矩阵(权威清单)

| omp 资产 | sdd-pack 内的目录/文件 | 作用 | 触发机制 |
| --- | --- | --- | --- |
| Skills | `skills/sdd` | 主 agent 看到 description -> 主动 read SKILL.md 加载流程知识 | description 触发,主 agent 自主加载 |
...
| Extension | `extensions/sdd-extension/index.ts` | 注册 `/sdd` slash command(18 子命令) + 拦截 tool_call(commit gate / session_start reminder / path gate) | `omp --extension <path>` 装载 |
...
| 硬门禁 (程序级) | `/sdd gate` 子命令返回 `status: "block"` | `extensions/sdd-extension/index.ts`（1 个 command） |
...
> link 后自动生效: `/sdd gate` 子命令 + 5 个 TTSR rule + tool_call 硬拦截 + skills。
...
| extension| `extensions/sdd-extension/index.ts`(1 个 `/sdd` 主命令 + 18 子命令) |

### 2.2 Slash Commands(1 个 `/sdd` + 子命令)
...
| `/sdd plan --phase <title>\|--link <phase-id>` | 已评审 -> 已规划任务 |
| `/sdd start` | 已规划任务 -> 进行中 |
| `/sdd archive --reason <completed\|abandoned>` | 归档 PRD |
...
| `/sdd apply <prd-path>` | 打印 PRD 验收标准 checklist |
| `/sdd validate [--path] [--staged] [--severity warn\|error\|block]` | 校验 docs/(11 项检查) |
...
| `/sdd gate <lint\|test\|review\|precommit\|commit> [args]` | 门禁流水线 |
...
- **extension 接口**:旧 `/sdd-*` 命令已移除,统一为 `/sdd <subcommand>`(详见 §2.2)
...
常见问题见本文档「开发模式」与「迁移」节;历史故障排查记录见 git history 中的 v1.4 README §故障排查。
