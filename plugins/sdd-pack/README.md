...
> **v1.1.0 起需要 `--hook` 装载**: omp v16.1.16 plugin 装载器不识别 `omp.hooks` 字段;hook 必须通过 CLI flag 显式加载(详见 ADR-006)。
> **v1.5.0-alpha 起 hook 二选一**: `plugins/sdd-pack/hooks/sdd/index.ts`(SDD 范式守卫)或 `plugins/sdd-pack/hooks/openspec/index.ts`(OpenSpec 范式守卫,详见 ADR-011);不装载 hook 时仅 extension 工作。
...
**v1.1.0 验证**: 启动 omp 时(`omp --hook plugins/sdd-pack/hooks/sdd/index.ts`),系统提示中应含「📜 lore 提交协议(... plugin hook 注入)」摘要。如未出现,检查: (1) `--hook` flag 路径正确;(2) `~/.omp/agent/rules/lore-protocol.md` 未被 native provider 覆盖(本版本 hook 与 native 共存,native 优先级更高)。
...
│       └── hooks/                         # v1.1.0 新增 — hook 聚合(ADR-011 双范式:二选一装载)
│           ├── sdd/                       # SDD 范式守卫(默认推荐)
│           │   └── index.ts               # 4 工厂聚合(session_start + tool_call)
│           └── openspec/                  # OpenSpec 范式守卫(可选)
│               └── index.ts               # 1 session_start + 1 tool_call 双分支
...
`hooks/sdd/index.ts` 的 `runSddValidate` 不再 spawn subprocess,改为 in-process 调用 `api.validateDocs()`:
...
> **范式选择**: OpenSpec 用户装 `hooks/openspec/index.ts`(无 in-process 校验,走 openspec-api-runner 跑 validate + status 双门控);SDD 用户装 `hooks/sdd/index.ts`(in-process api.validateDocs)。二者**互斥**装载,详见 ADR-011。
...
# 修改 plugins/sdd-pack/skills/<skill>/ 或 hooks/<sdd|openspec>/index.ts
...
1. 确认命令含 `--hook $(pwd)/plugins/sdd-pack/hooks/sdd/index.ts`(或 `hooks/openspec/index.ts`)
2. 路径用 `$(pwd)` 绝对化,避免 cwd 漂移
3. `bun build ./plugins/sdd-pack/hooks/sdd/index.ts --target=bun --outdir=/tmp/test` 确认 TS 编译通过
...
**升级前建议**: 备份 `plugins/sdd-pack/skills/sdd-*/SKILL.md` 的本地修改(若有);`hooks/sdd/index.ts` 大改时(>10 行)应单独发一个 minor 版本。
