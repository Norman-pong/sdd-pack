...
description: |
  ...
  状态流转触发词:"状态流转"、"PRD 评审"、"归档 PRD"、"phase 流转"、"/sdd"。
...
SDD 状态流转由 `/sdd` 命令家族统一驱动，核心命令如下：

- `/sdd init` — 创建新 PRD（草稿态）
- `/sdd review` — PRD 草稿 → 待评审
- `/sdd approve` — PRD 待评审 → 已评审（可配置触发 reviewer 门禁）
- `/sdd plan` — PRD 已评审 → 已规划任务
- `/sdd start` — PRD 已规划任务 → 进行中
- `/sdd back` — 合法回退（如 待评审 → 草稿）
- `/sdd archive` — 归档 PRD（已归档终态，不可逆）
- `/sdd phase complete` — Phase 完成 → 触发 lint+test 门禁
- `/sdd sync` — 检测 meta.json 与 markdown 不一致，并 `--fix` 重建

这些命令通过 `session_start` 注入的 `SDD_COMMAND_REMINDER` 自动提示，确保用户在任何会话开始时即可发现当前状态与可执行命令。用户提到"状态流转"、"PRD 评审"、"归档 PRD"、"phase 流转"、"/sdd" 时，应引导其使用上述命令。
