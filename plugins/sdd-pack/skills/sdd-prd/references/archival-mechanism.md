...
状态值域由 conventions.md §3.4 统一定义:**草稿 / 评审中 / 已评审 / 已归档 / 已替换 / 已废弃**。

sdd-prd 的归档机制**统一到** conventions.md §3.4 的状态值域(不再叠加第二套 HTML 注释标记):

| 维度     | conventions.md §3.4 定义                        | sdd-prd 触发                               |
| -------- | ----------------------------------------------- | ------------------------------------------ |
| 状态值域 | 草稿/评审中/已评审/已归档/已替换/已废弃         | sdd-prd 负责后三者的触发判定               |
| 文件位置 | `docs/prd/`(活跃) / `docs/prd/archive/`(已归档) | 归档时移动,替换/废弃原地标注               |
| 状态字段 | `> 状态: 已归档` 等(唯一标记)                   | 不使用 `<!-- archived -->` 等第二套标记    |
| 索引同步 | conventions.md §7 定义规则                      | 按 `rule://docs-update-guard` 在提交前同步 |

**核心原则**:状态字段是唯一事实来源——归档/替换/废弃都通过 `> 状态:` 表达,agent 加载时只读这一处。
...
> **状态:已替换** | 替换日期:2026-09-15 | 替代 PRD:2026-09-15-user-authentication-v2.md
>
...
> **状态:已废弃** | 废弃日期:2026-08-01 | 原因:项目延期
```
...
A: 不冲突。sdd-core 状态管"文档评审生命周期"(草稿/评审中/已评审),sdd-prd 归档状态管"目标驱动"(活跃/已归档/已替换/已废弃)。两者正交,各自维护。
