...
sdd 技能在对应产物目录下可创建 `.working/` 临时区，存放阶段性工作产物（问题清单、ADR 草稿、任务分解等）。约定：
...
PRD 状态值域（单一事实来源，sdd 归档机制必须以此为准）：

| 状态           | 含义                             | 物理位置            | 触发条件                        |
| -------------- | -------------------------------- | ------------------- | ------------------------------- |
| **草稿**       | 概念先行，无任何约束，可自由修改 | `docs/prd/`         | PRD 工作流阶段 1-3                |
| **待评审**     | 经过多轮沟通，格式/规范已正式    | `docs/prd/`         | PRD 工作流阶段 4 提交后           |
| **已评审**     | 评审通过，等待规划任务           | `docs/prd/`         | 评审通过                        |
| **已规划任务** | 任务已拆解到 phase/，待开始执行  | `docs/prd/`         | 任务拆解完成                    |
| **进行中**     | phase 任务正在执行               | `docs/prd/`         | 任务开始执行                    |
| **已归档**     | 终态，文件已移入 archive/ 目录   | `docs/prd/archive/` | 验收开关全勾 / 用户确认目标达成 |

**归档 = 状态字段变更 + 文件移动**，二者必须同步，不使用 HTML 注释等第二套标记：

- 归档：`> 状态：已归档` + 移动到 `docs/prd/archive/`
- 归档原因（ArchiveReason）在归档时标注：
  - 已完成：所有 phase 全部通过，目标达成
  - 已中止：不再继续推进
