...
> 本机制是 sdd 独家能力--sdd 本身的状态机基底不包含归档,本技能在状态机之上增加目标达成维度。
...
sdd 的归档机制**统一到** conventions.md §3.4 的状态值域(不再叠加第二套 HTML 注释标记):

| 维度     | conventions.md §3.4 定义                        | sdd 触发                                    |
| -------- | ----------------------------------------------- | ------------------------------------------ |
| 状态值域 | 草稿/待评审/已评审/已规划任务/进行中/已归档（含 ArchiveReason：已完成/已中止） | sdd 负责已归档（含 ArchiveReason）的触发判定 |
...
echo '{
  ...
  "body": "目标达成,移动-> docs/prd/archive/。需 sdd 同步 docs/index.md 移入归档分区。",
  "trailers": {
...
2. [ ] 读取 sdd "目标声明"--确认目标还在不在
3. [ ] 读取 sdd "目标验收开关"--是否全部勾选
...
graph TD
    ...
    Prd --> Readme["README.md<br/>(sdd 维护索引)"]
    Prd --> Archive["archive/"]
...
echo '{
  ...
  "trailers": {
    "Constraint": ["归档 PRD 必须移→ docs/prd/archive/"],
    "Directive": ["sdd 需同步 docs/index.md 移入归档分区"]
  }
}' | lore commit
```
...
### Q: sdd 的评审状态和归档维度冲突怎么办?

A: 不冲突。sdd 的评审状态管文档评审生命周期（草稿/待评审/已评审/已规划任务/进行中），归档维度管目标驱动（活跃/已归档，含 ArchiveReason：已完成/已中止）。同一状态机的两个轴。
