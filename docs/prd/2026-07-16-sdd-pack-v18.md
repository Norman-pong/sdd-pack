...
> 状态:待评审 | 创建日期:2026-07-16 | 系替代:[sdd-pack PRD (v1.7 整合)](archive/2026-07-16-sdd-pack.md)
> 修改记录:执行 `lore log docs/prd/2026-07-16-sdd-pack-v18.md`
> 对应阶段:TBD - 由 /sdd plan 补全
> 关键决策:[ADR-009 sdd Extension 替代独立 CLI](../architecture/decisions.md) · [ADR-013 omp 5 类资产分工契约](../architecture/decisions.md) · [ADR-015 hook 合并进 extension](../architecture/decisions.md) · [ADR-016 PRD 状态机重构](../architecture/decisions.md) · [ADR-017 Phase 状态机](../architecture/decisions.md) (Accepted) · **ADR-018 强状态流转 + meta.json 事实源 (TBD)**
...
| 决策 | ADR | 状态 | 说明 |
|------|-----|------|------|
| meta.json 为状态唯一事实源 | ADR-018 | TBD | markdown 状态行降级为展示层 |
| 全局单例 PRD | ADR-018 | TBD | 同时只 1 份活跃 PRD |
| /sdd 主命令+子命令体系 | ADR-018 | TBD | 替代 14 个分散命令 |
| tool_call 硬拦截状态行 | ADR-018 | TBD | write/edit 指向状态行被 block |
| PRD 1:N Phase ID 关联 | ADR-018 | TBD | meta.json parentId/phaseIds |
| 门禁嵌入流转流程 | ADR-018 | TBD | 不独立暴露 /sdd-gate-* |
| meta.json 不进 git | ADR-018 | TBD | 本地状态,markdown 进 git |
| 移除 OpenSpec 双范式 | ADR-018 | TBD | 吸收 3 个精华到 SDD,删除 ~1500 行死代码 |
| Phase 按 PRD ID 分组目录 | ADR-018 | TBD | 吸收 OpenSpec 目录内聚,文件系统结构表达归属 |
...
| v1.8.0-alpha | TBD | meta-store.ts + /sdd init/review/approve 基础流转 |
| v1.8.0-beta | TBD | /sdd plan/start/archive + Phase 流转 + tool_call 硬拦截 |
| v1.8.0 | TBD | /sdd sync 迁移 + 旧命令别名 + validator 事实源切换 + 全链路测试 |
