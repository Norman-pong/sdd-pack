...
CLI 定位：**将 rule 中的判断逻辑程序化，将 sdd 中的操作流程自动化，成为 sdd-pack 文档生命周期的唯一权威入口。**
...
- **认知一致**：整个生态已用 `sdd` 指代这套体系（`sdd` / `sdd-pack` / `sdd-reviewer`）
...
| 组件                       | 职责                                  | CLI 是否替代                           | 关系                                          |
| -------------------------- | ------------------------------------- | -------------------------------------- | --------------------------------------------- |
| `sdd propose`              | 创建 PRD / delta                      | ✅ 替代手动 `write` + sdd 前几步   | 程序化 sdd 模板填充                       |
...
| `sdd` (skill)              | 文档创建/修改全流程指导               | ⚠️ 自动化                              | CLI 将其操作流程自动化                        |
...
2. **skill 不能替代 CLI**：`sdd` 指导 agent 操作，但依赖 agent 正确执行；agent 可能遗漏步骤
