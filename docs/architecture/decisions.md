...
| `docs-update-guard.md`  | -           | `(git\|lore)\s+commit` | `tool:bash`                | `pi.on("tool_call")` 匹配 `git\|lore commit`,`pi.sendMessage` 提示走 sdd | 不拦截,仅提示                              |
| `lore-commit-guard.md`  | —           | `(git\|lore)\s+commit` | `tool:bash`                | `pi.on("tool_call")` 匹配,`return { block: true, reason }` 硬拦截             | 强制用 lore commit                         |
| `sdd-doc-edit-guard.md` | -           | -                      | `tool:write/edit(docs/**)` | `pi.on("tool_call")` 匹配 `docs/`,`return { block: true, reason }`            | docs/ 写入走 sdd skill                |
...
- 同样方式读 `skill://sdd` 报 `Unknown skill`,available 列表仅含本机 managed-skills,**0 个 plugin skill**
...
ADR-015（v1.6.0）起 hook 逻辑合并进 extension，`pi.on('tool_call')` 的 `{block:true}` 已提供 lore-commit-guard 所需的程序级拦截能力。ADR-006 "hook 当前需保留为不可替代拦截层"的结论已由 extension 承接；独立 `hooks/` 目录已于 v1.6.0 删除（对应上方迁移路线阶段 d 提前完成）。ADR-006 保留 Accepted（历史决策如实记录）。
...
- **sdd-reviewer** 不评审代码质量(那是 reviewer/arch-reviewer 的活),不写文档(那是 sdd 的活)
...
sdd-pack v1.2.3 验证中发现 PRD 状态行堆叠（5 个版本功能挤在一条状态行）是文档管理的主要痛点。`prd-change-management` rule 提供了"是否要拆版本"的判断逻辑，但 rule 无法执行文件操作（移动、重命名、批量链接更新）。`sdd` skill 依赖 agent 正确执行操作步骤，agent 可能遗漏步骤。
...
2026-06-30 omp 生态市场调研结论（详见 [`docs/reference/archive/omp-extension-api.md`](../reference/archive/omp-extension-api.md)）：
...
- hook 只做写入提示（检测到写 architecture/ 或 reference/ 时提示走 sdd 流程），不做状态机校验
