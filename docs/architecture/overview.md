...
本文档描述 sdd-pack 仓库的架构定位、目录布局与分发机制。sdd-pack 是一个 omp marketplace 插件仓库，本身的"产品"是一个插件（`plugins/sdd-pack/`），其内容是 4 个 SDD skills + 4 个 rules + docs-check.sh。

## 1. 系统定位

sdd-pack 的核心定位：**SDD 技能家族 + lore 提交规则的版本化分发容器**。通过 omp marketplace 机制，让其他用户用 `omp plugin install sdd-pack@sdd-pack` 一条命令获得完整的 SDD 工具链。
...
| 最终用户 | 任意 omp 环境 | `omp plugin marketplace add norman/sdd-pack` + `install` |
| omp-plugins provider | `~/.omp/plugins/cache/...` | 加载 plugin 内的 skills/ + rules/ |
...
| rules 内容 | 4 个规则（lore-protocol + 3 个 guard） | `plugins/sdd-pack/rules/` |
...
- **plugin 目录**内 skills/ 与 rules/ 平级，omp-plugins provider 分别发现。
...
| rules 发现 | omp-plugins provider | priority 90（**需验证**） |
...
- 版本号：1.0.0
- 发布日期：TBD
- 内容：4 skills + 4 rules + docs-check.sh

### 10.2 演进路线

- [ ] 阶段 1 验证：rules 通过 omp-plugins provider 被发现（关键风险）
- [ ] 若验证失败：fallback 为 hook extension 模式（需 npm install）
