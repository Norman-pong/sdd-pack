...
| 任务 ID | 任务名称 | 预估 | 依赖 | 状态 |
|---------|---------|------|------|------|
| T01 | ADR-018 撰写并 Accepted | 15 min | — | 未开始 |
| T02 | meta-store.ts 类型定义 + 实现 | 40 min | T01 | 未开始 |
| T03 | meta-store 测试 | 25 min | T02 | 未开始 |
| T04 | api.ts 新增流转函数(init/review/approve/back) | 40 min | T02 | 未开始 |
| T05 | doc-parser.ts 新增 generateStatusLine | 15 min | T02 | 未开始 |
| T06 | extension /sdd 命令路由骨架 + init/review/approve/back handler | 60 min | T04, T05 | 未开始 |
| T07 | .gitignore 加 .sdd/meta/ | 2 min | — | 未开始 |
| T08 | OpenSpec extension + CLI 代码删除 | 15 min | — | 未开始 |
| T09 | marketplace.json 清理 OpenSpec 声明 | 10 min | T08 | 未开始 |
| T10 | session_start 移除 OpenSpec reminder | 5 min | T08 | 未开始 |
| T11 | ADR-010/011 标记 Superseded | 5 min | T08 | 未开始 |
| T12 | bun test + tsc 验证 | 10 min | T01-T11 | 未开始 |
...
| 里程碑 | 交付物 | 状态 |
|--------|--------|------|
| M0: meta-store 基础 | meta-store.ts + 测试 + generateStatusLine + .gitignore | 未达成 |
| M1: PRD 基础流转 | api.ts 4 个流转函数 + extension init/review/approve/back handler | 未达成 |
| M2: OpenSpec 清除 | 代码删除 + marketplace 清理 + ADR Superseded | 未达成 |
